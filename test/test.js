...
describe('phonix', function() {
        ...
        it('Should send a POST request with FormData data correctly (multipart)', function(done) {
            if(isNode) {
                ...
                testSetup(function(errors, window) {
                    ...
                    window.phonix.post('http://localhost:43760/api/post/form', formData).then(function(res) {
    ...
    describe('HEAD', function() {
        it('Should send a HEAD request correctly', function(done) {
            testSetup(function(errors, window) {
                window.phonix.head('http://localhost:43760/api/head', {details: true}).then(function({res, status, request}) {
    ...
    describe('Caching', function() {
        ...
        it('Should allow users to create custom caches with cache timeouts and clearing capabilities', function(done) {
            testSetup(function(errors, window) {
                ...
                window.phonix.get('http://localhost:43760/api/get', {cache: cacheOptions}).then(function(res) {
                    assert.equal(res.data, "GET");
    ...
    describe('XSRF', function() {
        it('Should send the correct XSRF Cookie', function(done) {
            testSetup(function(errors, window) {
                ...
                window.phonix.setCookie('XSRF-TOKEN', 'MY_XSRF_TOKEN');

                window.phonix.get('http://localhost:43760/api/xsrf').then(function(res) {
    ...
    describe('CORS', function() {
        ...
        it('Should send CORS requests correctly when CORS is enabled on server', function(done) {
            if(isNode) {
                ...
                testSetup(function(errors, window) {
                    window.phonix.get('http://localhost:43761/api/cors').then(function(res) {
        ...
        it('Should return an error when CORS requests are not allowed from this client', function(done) {
            if(isNode) {
                ...
                testSetup(function(errors, window) {
                    window.phonix.get('http://localhost:43761/api/cors/fail').then(function(res) {
                        //Do nothing
